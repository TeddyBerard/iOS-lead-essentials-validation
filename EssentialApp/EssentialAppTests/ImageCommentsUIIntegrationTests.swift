...
final class ImageCommentsUIIntegrationTests: XCTestCase {
	...
	func test_loadCommentActions_requestCommentsFromLoader() {
		...
		XCTAssertEqual(loader.loadRequestsCallCount, 0, "Expected no loading requests before view appears")

		sut.simulateAppearance()
		XCTAssertEqual(loader.loadRequestsCallCount, 1, "Expected a loading request once view appears")

		sut.simulateUserInitiatedReload()
		XCTAssertEqual(loader.loadRequestsCallCount, 2, "Expected another loading request once user initiates a reload")

		sut.simulateUserInitiatedReload()
		XCTAssertEqual(loader.loadRequestsCallCount, 3, "Expected yet another loading request once user initiates another reload")
	}
	...
	func test_loadCommentActions_runsAutomaticallyOnlyOnFirstAppearance() {
		...
		sut.simulateAppearance()
		XCTAssertEqual(loader.loadRequestsCallCount, 1, "Expected no loading request the second time view appears")
	}
